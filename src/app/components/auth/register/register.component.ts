...
{name: 'Zimbabwe', code: 'ZW'}
  ...
  register = this.fb.group({
    ...
    referrer = '';
  constructor(private fb: FormBuilder , private auth: AuthService, public router: Router, public route: ActivatedRoute,
    ...
    if (password === confirmPassword) {
      const country = this.register.get('country').value;
       const registerData = {
        ...
        exchangeName: this.register.get('userName').value,
        firstName: this.register.get('firstName').value,
        lastName: this.register.get('lastName').value,
        email: this.register.get('email').value,
        country: this.country.find(data => data.name === country),
        birthday: this.register.get('birthday').value,
        gender: this.register.get('gender').value,
        confirmPassword: this.register.get('confirmPassword').value,
        password: this.register.get('password').value,
        privacyPolicy: this.register.get('privacyPolicy').value,
        termsAndConditions: this.register.get('termsAndConditions').value,
        referrer: null
      };
       if (this.referrer) {
           registerData.referrer = this.referrer;
       }
      // @ts-ignore
      this.auth.registerUser(registerData).pipe(catchError((err) => {
        if (err.status === 400) {
        ...
        this.open(this.mymodal, {title:"Success", content:(<any>res).message});
